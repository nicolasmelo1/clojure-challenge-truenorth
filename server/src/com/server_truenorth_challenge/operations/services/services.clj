...
(defn- handle-expression-type-of-operation [database expression cost-and-id-by-operation user-id user-balance]
  (try
      ...
      (if does-user-have-enough-money?
        (try
          (let [{:keys [node amounts]} (interpreter/evaluate abstract-syntax-tree user-balance)
                _ (if (> (count amounts) 0) (operations-repository/records-bulk-insert database user-id (reverse amounts)), nil)]
            {:is-valid true
             :reason nil
             :data {:result node
...
(defn- handle-random-string-type-of-operation [database cost-and-id-by-operation user-id user-balance get-random-string-callback]
  (try (let [data (get-random-string-callback)
             new-user-balance (- user-balance (-> cost-and-id-by-operation :random_string :cost))]
         (if (>= new-user-balance 0)
           (let [_ (operations-repository/records-bulk-insert database user-id [{:operation-type :random_string
...
(defn new-operation [database operation-type expression user-id get-random-string-callback]
  (let [all-operations (operations-repository/operations-get-all database)
        ...
        previous-record (operations-repository/records-get-last-record-of-user database user-id)
    ...
    (if (= operation-type "expression")
      (handle-expression-type-of-operation database expression cost-and-id-by-operation user-id user-balance)
      (handle-random-string-type-of-operation database cost-and-id-by-operation user-id user-balance get-random-string-callback))))


(defn get-all-operations
  "Returns a list of all of the operations available to be performed by the user"
  []
  (operations-repository/operations-get-all settings/db))